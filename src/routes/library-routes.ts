...
export default function libraryRoutes(server, options, done) {
	...
	server.get("/", async (req, res) => {
		const getBook = await libRepo.find();
	...
	server.post("/", async (req, res) => {
		...
		if (getBook == null) {
			...
			console.log(
				"New Book posted in the library ",
				postBook
			);

			return postBook;
		} else {
			throw new Error("The Book is already in the library");
		}
	});
	...
	server.put("/", async (req, res) => {
		...
		const updateBook = await libRepo.update(bookId, {
			bookName,
			authorName,
			language,
			genre,
	...
	server.delete("/", async (req, res) => {
		...
		const getBook = await libRepo.findOne({
			where: { ...queryParams },
		});

		if (getBook != null) {
			const deleteBook = await libRepo.delete({...queryParams});
